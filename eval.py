...
def main():
    ...
    if args.dataset_name is not None:
        ...
        if args.has_answer_file is not None:
            data_files["has_answer"] = args.has_answer_file
        if args.no_answer_file is not None:
            data_files["no_answer"] = args.no_answer_file
        if args.test_file is not None:
    ...
    if args.validation_file is not None:
        ...
        data_files["validation"] = args.validation_file
        extension = args.validation_file.split(".")[-1]
        eval_datasets = load_dataset(extension, data_files=data_files, field="data")
        raw_datasets['validation'] = eval_datasets['validation']
    
    if args.has_answer_file is not None and args.no_answer_file is not None:
        data_files = {}
        data_files['has_answer'] = args.has_answer_file
        data_files['no_answer'] = args.no_answer_file
        extension = args.has_answer_file.split(".")[-1]
        eval_datasets = load_dataset(extension, data_files=data_files, field="data")
        raw_datasets['validation'] = concatenate_datasets([eval_datasets['has_answer'], eval_datasets['no_answer']])
        #raw_datasets['validation'] = eval_datasets['no_answer']
        #pdb.set_trace()
